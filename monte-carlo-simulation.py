...
for _, method in non_best_methods.iterrows():
    ...
    st.write(f"- Project duration is reduced by **{time_saved:.2f} months** ({time_savings_percentage:.1f}%).")
    st.write(f"- Project cost is reduced by **{cost_saved:.2f} million dollars** ({cost_savings_percentage:.1f}%).")
